import { UpdateUserRequest, UserSettings } from '@amal-ia/lib-types';
...
export const setCurrentUser: ActionCreator<ReduxAction> = (user: User | null) => ({
...
export const updateCurrentUserSettings = (settings: UserSettings): ThunkResult<Promise<ReduxAction>> => async (dispatch) => {
  dispatch(usersStart());
  try {
...
export const setUsers: ActionCreator<ReduxAction> = (users: User[], allUsers: boolean, activeUsers: boolean) => ({
...
export const fetchUsers = (
  userIds: string[],
): ThunkResult<Promise<ReduxAction>> => async (dispatch, getState) => {
  ...
  try {
    const fetchedUsers = await UsersRepository.fetchUsersByIds(uniq(usersToFetch));
    // We need to pass already fetched users as some components need the whole list
    return dispatch(setUsers([
      ...Object.values(currentUsersMap),
      ...fetchedUsers,
    ]));
  } catch (error) {
    return dispatch(usersError(error));
...
export const fetchAllUsers = (onlyActive: boolean): ThunkResult<Promise<ReduxAction>> => async (
  dispatch,
  getState,
) => {
